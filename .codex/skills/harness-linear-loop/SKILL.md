...
Use the configured Linear MCP server or Linear API. If neither is available, write the missing auth/tool blocker into `## Codex Workpad` and stop.

The loop maintains:

- status: `Todo -> In Progress -> Human Review -> Merging -> Done`, with `Rework` for reviewer changes.
- single source of truth: one persistent `## Codex Workpad`.
...
0. Open `.codex/skills/harness-quality-gate/SKILL.md` and `.codex/skills/using-superpowers/SKILL.md` before other work.
...
3. Create or update `## Codex Workpad` with plan, acceptance criteria, validation checklist, and environment stamp.

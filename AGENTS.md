...
4. SDD 是实现前置门禁：涉及架构、数据模型、接口、状态机、队列、权限或 RAG 的任务，先写/更新设计，再写代码。
5. 修改核心逻辑时默认 TDD：先红灯测试或可执行验收，再最小实现，最后在绿灯保护下重构。
6. RAG 相关能力必须有检索质量测试，覆盖索引、召回、排序、引用、空结果和降级路径。
7. 无法先写测试时，必须说明原因，并提供替代验证证据。
8. 只改本任务相关文件；发现有价值但越界的事项，另建 Backlog 任务，不扩大当前范围。
9. 单文件长期目标不超过 200 行；确需更长时按职责拆分。

## SDD / TDD / RAG 门禁

1. SDD 至少说明目标、非目标、数据/接口契约、状态流、失败路径、权限边界、验证方式和迁移/回滚影响。
2. TDD 必须绑定验收标准：先证明问题或需求，再写最小实现；`test:` commit 先于 `impl:`/`feat:` commit。
3. RAG 测试必须使用稳定 fixture 或小型黄金集，验证 chunking、embedding/index 更新、top-k 召回、重排、去重、引用溯源和无命中降级。
4. RAG 输出不得只测“有文本返回”；必须检查证据是否来自检索结果、引用是否可追踪、低置信度时是否拒答或降级。
5. 涉及 SDD/TDD/RAG 的 ticket，Workpad 和 PR 都必须记录对应设计链接、红绿证据和测试命令。
...
2. 绿灯阶段只写让测试通过的最小代码。
3. 重构阶段不得改变已验证行为。
4. 测试优先覆盖核心业务规则、边界条件、回归缺陷、RAG 质量和 ticket 验收标准。

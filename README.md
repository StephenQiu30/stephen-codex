...
本项目把 Codex 项目中常见的规范文件、角色分工、OpenSpec change 流程、TDD/SMART/MVP 原则、Symphony-ready ticket 编排、docs 文档分类和 Git/PR 收口要求整理为一套可直接复用的目录模板。你可以把它作为新项目的起点，也可以复制其中的规范文件到已有项目中逐步落地。
...
6. 如何通过 `WORKFLOW.md` 对齐 OpenAI Symphony 的 Linear ticket 编排方式。
...
5. 新增文档时遵循 `docs/` 分类规则，不同类型文档写入不同子目录。
6. 在已有 Codex 项目中使用时，可以直接复制 `AGENTS.md`、`AGENTS.local.md`、`WORKFLOW.md`、`.codex/`、`docs/` 和 `openspec/`。
...
5. `Git 收口`：每次使用 OpenSpec 执行完一个较大的 change 后，必须完成测试、验证、归档和归档后校验，再使用中文 Git 提交信息提交，并保持工作区干净。
...
4. 完成后应测试并验证 change，记录验证结果。
5. 验收通过后应归档 change，并在归档后再次执行 OpenSpec 校验。
6. 归档和校验完成后，才能使用中文 Git 提交信息提交，并保持工作区干净。
